...
.dmodal{{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;padding:18px}}.dmodal[hidden]{{display:none}}.dback{{position:absolute;inset:0;background:rgba(10,14,22,.62);backdrop-filter:blur(3px)}}.dbox{{position:relative;width:min(860px,100%);max-height:min(82vh,820px);display:flex;flex-direction:column;background:var(--panel);border:1px solid var(--line);border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.32);overflow:hidden}}.dhead{{display:flex;justify-content:space-between;align-items:center;padding:17px 20px;border-bottom:1px solid var(--line)}}.dhead b{{font:800 22px 'Barlow Condensed'}}.dhead span{{display:block;color:var(--mut);font-size:12px}}.dclose{{border:0;background:var(--panel2);color:var(--txt);width:36px;height:36px;border-radius:50%;font-size:24px;cursor:pointer}}.dtools{{display:flex;align-items:center;gap:10px;padding:12px 16px;background:var(--panel2)}}.dtools input{{min-width:0;border:1px solid var(--line);border-radius:9px;padding:9px 12px;background:var(--panel);color:var(--txt);font:600 13px Manrope}}#dsearch{{flex:1}}#ddate{{flex:0 0 155px}}#dclear{{border:1px solid var(--line);border-radius:9px;padding:9px 11px;background:var(--panel);color:var(--mut);font:700 12px Manrope;cursor:pointer}}.dtools b{{white-space:nowrap}}.dlist{{overflow:auto;padding:8px 14px 18px}}.drow{{display:grid;grid-template-columns:1fr auto;gap:5px 14px;padding:11px 6px;border-bottom:1px solid var(--line)}}.dnm{{font-weight:800}}.dmeta{{font-size:12px;color:var(--mut)}}.dst{{align-self:center;padding:4px 9px;border-radius:20px;font:800 11px Manrope}}.dst.paid{{background:rgba(61,255,162,.16);color:var(--greentx)}}.dst.debt{{background:rgba(255,79,40,.14);color:var(--redtx)}}.dst.archive{{background:rgba(128,138,153,.20);color:#687386}}.damt{{font:700 12px Manrope;color:var(--mut);text-align:right}}.dempty{{padding:35px;text-align:center;color:var(--mut)}}@media(max-width:700px){{.dtools{{flex-wrap:wrap}}#dsearch{{flex-basis:100%}}#ddate{{flex:1}}.dmodal{{padding:0;align-items:end}}.dbox{{max-height:90vh;border-radius:18px 18px 0 0}}}}</style>
</head><body>
...
(function(){var QS=new URLSearchParams(location.search);
 ...
 var dm=document.getElementById('dmodal'),dl=document.getElementById('dlist'),ds=document.getElementById('dsearch'),dd=document.getElementById('ddate'),dclear=document.getElementById('dclear'),dc=document.getElementById('dcount'),dsub=document.getElementById('dsub'),current=[];
 ...
 function draw(){var term=(ds&&ds.value||'').trim().toLowerCase(),dv=(dd&&dd.value||'').split('-'),picked=dv.length===3?(dv[2]+'.'+dv[1]+'.'+dv[0]):'',arr=current.filter(function(x){return (!term||String(x.name||'').toLowerCase().indexOf(term)>=0)&&(!picked||x.period===picked);}).slice().sort(function(a,b){var d=dateKey(a.period).localeCompare(dateKey(b.period));return d||String(a.name||'').localeCompare(String(b.name||''),'uz');});if(dc)dc.textContent=arr.length+' ta';if(!dl)return;dl.innerHTML=arr.length?arr.map(function(x){var paid=['paid','bit','referral'].indexOf(x.bucket)>=0,cls=x.bucket==='deleted'?'archive':(paid?'paid':'debt');return '<div class="drow"><div><div class="dnm">'+escH(x.name)+'</div><div class="dmeta">'+escH(x.curator||'Biriktirilmagan')+'</div></div><span class="dst '+cls+'">'+escH(x.status)+'</span><div class="dmeta">'+escH(x.period||'')+'</div><div class="damt">'+money(paid?x.paid:x.debt)+'</div></div>';}).join(''):'<div class="dempty">Ma’lumot topilmadi</div>';}
 function escH(s){return String(s==null?'':s).replace(/[&<>\"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c];});}
 function openDetail(btn){var kind=btn.getAttribute('data-list')||'plan',cur=btn.getAttribute('data-curator')||'',cash=btn.getAttribute('data-cashier')||'',curs=(btn.getAttribute('data-curators')||'').split('|').filter(Boolean);current=DETAIL.filter(function(x){return (!cur||x.curator===cur)&&(!cash||x.cashier===cash)&&(!curs.length||curs.indexOf(x.curator)>=0)&&(kind==='plan'||(kind==='paid'&&['paid','bit','referral'].indexOf(x.bucket)>=0)||(kind==='debt'&&(x.bucket?x.bucket==='debt':x.status!=="To'lagan")));});if(dsub)dsub.textContent=((cur||cash)?(cur||cash)+' · ':'')+(kind==='paid'?'To‘landi + Bitirdi + Referal':kind==='debt'?'Qarzdor':'PLAN tarkibi');if(ds)ds.value='';if(dd)dd.value='';draw();if(dm){dm.hidden=false;document.body.style.overflow='hidden';if(ds)ds.focus();}}
 document.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('[data-list]');if(b){openDetail(b);return;}if(e.target.closest&&e.target.closest('[data-close-detail]')){dm.hidden=true;document.body.style.overflow='';}});if(ds)ds.addEventListener('input',draw);if(dd)dd.addEventListener('change',draw);if(dclear)dclear.addEventListener('click',function(){if(dd)dd.value='';draw();});document.addEventListener('keydown',function(e){if(e.key==='Escape'&&dm&&!dm.hidden){dm.hidden=true;document.body.style.overflow='';}});
}());
